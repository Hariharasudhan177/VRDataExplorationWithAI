...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    //CAS Implmentation 
    public UnityEngine.XR.XRNode leftOrRightHand;
    private bool radialMenuActive = false;
    private bool primaryButtonPressed = false;
    private bool primaryButtonReleased = false;

    //to check click and unclick change to events later
    bool touchedPadClicked = false;
    bool waitingForTouchPadUnclick = false;
    ...
    void Update() {
        ...
        //CAS Implementation
        var inputXRDevice = new List<UnityEngine.XR.InputDevice>();
        UnityEngine.XR.InputDevices.GetDevicesAtXRNode(leftOrRightHand, inputXRDevice);


        bool primaryButtonBeingPressed = false;
        if (inputXRDevice.Count > 0)
        {
            if (inputXRDevice[0].TryGetFeatureValue(UnityEngine.XR.CommonUsages.primaryButton, out primaryButtonBeingPressed) && primaryButtonBeingPressed)
            {
        ...
        if(primaryButtonPressed && primaryButtonReleased)
        {
            ...
            this.gameObject.GetComponent<CanvasGroup>().interactable = radialMenuActive;
        }


        if (radialMenuActive)
        {
            bool primaryAxisTouched = false; 
            if(inputXRDevice[0].TryGetFeatureValue(UnityEngine.XR.CommonUsages.primary2DAxisTouch, out primaryAxisTouched) && primaryAxisTouched)
            {
                Vector2 primary2DAxisPosition;
                inputXRDevice[0].TryGetFeatureValue(UnityEngine.XR.CommonUsages.primary2DAxis, out primary2DAxisPosition);

                bool primary2DAxisClicked;
                inputXRDevice[0].TryGetFeatureValue(UnityEngine.XR.CommonUsages.primary2DAxisClick, out primary2DAxisClicked);

                if (primary2DAxisClicked)
                {
                    if (!waitingForTouchPadUnclick)
                    {
                        touchedPadClicked = true;
                        waitingForTouchPadUnclick = true;
                    }
                    else
                    {
                        touchedPadClicked = false;
                    }
                }
                else
                {
                    touchedPadClicked = false;
                    waitingForTouchPadUnclick = false;
                }

                if (primary2DAxisPosition.x > 0)
                {
                    if (primary2DAxisPosition.y >= 0)
                    {
                        selectButton(0);

                        if (touchedPadClicked)
                        {
                            ExecuteEvents.Execute(elements[0].button.gameObject, pointer, ExecuteEvents.submitHandler);
                        }
                    }
                    else
                    {
                        ...
                        if (touchedPadClicked)
                        {
                            ExecuteEvents.Execute(elements[1].button.gameObject, pointer, ExecuteEvents.submitHandler);
                        }
                    }
                }
                else if (primary2DAxisPosition.x < 0)
                {
                    if (primary2DAxisPosition.y <= 0)
                    {
                        selectButton(2);
                        ...
                        if (touchedPadClicked)
                        {
                            ExecuteEvents.Execute(elements[2].button.gameObject, pointer, ExecuteEvents.submitHandler);
                        }
                    }
                    else
                    {
                        ...
                        if (touchedPadClicked)
                        {
                            ExecuteEvents.Execute(elements[3].button.gameObject, pointer, ExecuteEvents.submitHandler);
                        }
                    }
                }
            }
            ...
            {
                deSelectButton(previousActiveIndex);
            }       
        }
        else
        {
    ...
    //CAS Implementation
    //Deselects the button with the specified index.
